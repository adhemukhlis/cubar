...
export const Store_GuestData = async (guestdata) => {
	return DataUser(guestdata.uid, guestdata.username, guestdata.imageProfile, guestdata.email)
}
...
export const Store_SetBasicUserData = (id, data) => firebaseRefUser.child(id).update(data)
export const Store_UserData = (userdata, callback) =>
	firebaseRefUserFind(userdata.uid).on('value', (snap) => {
		if (snap.exists()) {
			...
			const data = DataUser(userdata.uid, userdata.username, userdata.imageProfile, userdata.email)
...
export const Store_GetGameData = (game_name, callback) =>
	firebaseRefGameData(game_name).once('value', (snap) => {
		...
		if (game_name === 'simplicity') {
			...
			snap.forEach((shot) => {
				data.push(DataOperations(shot.val().a, shot.val().b, shot.val().o, shot.val().r))
			})
		}
		setState({ [game_name]: data })
		// callback( data );
	})
export const Store_SetUserScore = (id, cat, val) => firebaseRefFindUserScore(id).update({ [cat]: val })
...
export const Store_SetUserRoomcode = (id, roomcode) => firebaseRefUserDetail(id).update({ roomcode: roomcode })
export const Store_DelRoomGameline = (roomcode) => firebaseRefRoomGameline(roomcode).remove()
export const Store_UpdateRoom = (roomcode, gameline) => firebaseRefRoom(roomcode).update({ gameline: gameline })
export const Store_SetRespondenData = (uid, nama, email, umur, gender, pendidikan) =>
...
export const Store_PushRespondenData = (key, nama, umur, gender, sekolah, kelas, func) => {
...
export const Store_SetRespondenSectionData = (section, uid, data) => firebaseRespondenSectionRef(uid, section).set(data)
